...
export function BeatmapsetCard({ beatmapset, playing, selected, onOpen, onPreview, onSelect }: { beatmapset: OnlineBeatmapset; playing: boolean; selected: boolean; onOpen: () => void; onPreview: () => void; onSelect: () => void }) {
  ...
  return <Card className={`group overflow-hidden transition ${selected ? "border-cyan-300/30 bg-cyan-300/[0.045] shadow-[0_14px_44px_rgba(92,225,230,.08)]" : "hover:border-white/[0.13] hover:bg-[#121a28]"}`}>
    <div className="flex min-h-[150px]">
      <button aria-label={selected ? "从下载队列移除" : "加入下载队列"} className="relative w-40 shrink-0 overflow-hidden bg-[#090d17] text-left" disabled={disabled} onClick={onSelect} type="button">
