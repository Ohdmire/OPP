import { Check, Download, Headphones, Info, ListPlus, Pause } from "lucide-react";
...
export function BeatmapsetCard({ beatmapset, downloading, playing, selected, onDownload, onOpen, onPreview, onSelect }: { beatmapset: OnlineBeatmapset; downloading: boolean; playing: boolean; selected: boolean; onDownload: () => void; onOpen: () => void; onPreview: () => void; onSelect: () => void }) {
  ...
  return <Card className={`group cursor-pointer overflow-hidden transition ${selected ? "border-cyan-300/30 bg-cyan-300/[0.045] shadow-[0_14px_44px_rgba(92,225,230,.08)]" : "hover:border-white/[0.13] hover:bg-[#121a28]"}`} onClick={onOpen} role="button" tabIndex={0}>
    <div className="flex min-h-[150px]">
      <button aria-label={selected ? "从下载队列移除" : "加入下载队列"} className="relative w-40 shrink-0 overflow-hidden bg-[#090d17] text-left" disabled={disabled} onClick={(event) => { event.stopPropagation(); onSelect(); }} type="button">
        ...
        <div className={`absolute left-3 top-3 grid size-8 place-items-center rounded-lg border backdrop-blur ${selected ? "border-cyan-200/50 bg-cyan-300/80 text-[#071017]" : "border-white/20 bg-black/45 text-white"}`}>{selected ? <Check className="size-4" /> : <ListPlus className="size-4" />}</div>
