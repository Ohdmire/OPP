...
export function BeatmapDownloadPanel({
  ...
  const [provider, setProvider] = useState<"hinai" | "catboy" | "nerinyan" | "none">("hinai");
  ...
  const percent = useMemo(
    () => progress?.total ? Math.min(100, (progress.processed / progress.total) * 100) : 0,
    [progress],
  );
    ...
    <>
      <Card className="sticky top-[120px] overflow-hidden">
        ...
        <div className="space-y-5 p-5">
          ...
          <label className="block"><span className="mb-2 block text-sm font-medium text-slate-300">下载适配器</span><select className="w-full rounded-xl border border-white/[0.09] bg-[#0b101b] px-3 py-3 text-sm text-slate-200 outline-none focus:border-cyan-300/45" disabled={busy || collecting} onChange={(event) => setProvider(event.target.value as typeof provider)} value={provider}><option value="hinai">Hinai Mirror（推荐，多源回退）</option><option value="catboy">Catboy</option><option value="nerinyan">Nerinyan</option><option value="none">不使用镜像</option></select></label>
          ...
          {progress ? <div className="rounded-xl border border-white/[0.08] bg-black/15 p-4"><div className="flex items-center justify-between gap-3 text-sm"><span className="max-w-[180px] truncate text-slate-400">{progress.current_title ?? progress.message ?? "正在下载"}</span><span className="font-mono text-[var(--theme-primary)]">{progress.processed}/{progress.total}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full rounded-full bg-[var(--theme-primary)] transition-all" style={{ width: `${percent}%` }} /></div><div className="mt-3 flex items-center justify-between text-xs text-slate-500"><span className="inline-flex items-center gap-1.5"><Gauge className="size-3.5" />{progress.phase === "finished" ? "下载完成" : progress.phase === "cancelled" ? "已取消" : "下载中"}</span><span>{(progress.bytes_per_second ?? 0) > 0 ? `${((progress.bytes_per_second ?? 0) / 1024 / 1024).toFixed(1)} MB/s` : "等待速度"}</span></div></div> : null}
