...
export function GameSessionPage() {
  ...
  useEffect(() => { const initial = window.setTimeout(() => { void refresh(); void desktopApi.getSettings().then((settings) => setLaunchTosu(settings.launch_tosu_with_game)); }, 0); const timer = window.setInterval(() => void refresh(), 2500); return () => { window.clearTimeout(initial); window.clearInterval(timer); }; }, []);
  ...
  const launch = async () => { setBusy(true); setError(null); try { setSession(await desktopApi.startGameSession(ruleset, targetClient, launchTosu)); } catch (value) { setError(value); } finally { setBusy(false); } };
  return <>
    <div className="mb-4 flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.025] px-4 py-3"><span className="text-xs text-slate-500">启动客户端</span><div className="flex items-center gap-5"><label className="flex cursor-pointer items-center gap-2 text-xs text-slate-300"><input checked={launchTosu} className="accent-cyan-300" onChange={(event) => void updateLaunchTosu(event.target.checked)} type="checkbox" />同时启动 tosu</label><ClientSwitch value={targetClient} onChange={setTargetClient} /></div></div>
