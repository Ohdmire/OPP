...
import { BarChart3, ExternalLink, FileChartColumn, Gauge, Gamepad2, Image, Map, Music2, Palette, Settings } from "lucide-react";
...
export function Sidebar({ profile, loading }: { profile?: OwnProfile; loading: boolean }) {
  ...
  const startGame = async () => { setStarting(true); try { await desktopApi.startGameSession(ruleset, client); } finally { setStarting(false); } };
  return <aside className="fixed bottom-0 left-0 top-11 z-40 flex w-[224px] flex-col border-r border-white/[0.06] bg-[#0a0e18]/94 px-3 py-5 backdrop-blur-xl">
    <nav aria-label="主导航" className="flex min-h-0 flex-1 flex-col"><GroupLabel>在线资料</GroupLabel><div className="space-y-1">{onlineLinks.map((link) => <NavItem key={link.to} {...link} />)}</div><div className="mt-7"><GroupLabel>本地资源</GroupLabel><div className="space-y-1"><NavItem icon={Map} label="本地谱面" to="/local/maps" /><NavItem icon={Palette} label="本地皮肤" to="/local/skins" /><NavItem icon={Image} label="截图回放" to="/local/media" /></div></div></nav>
    <div className="flex items-center gap-1"><div className="min-w-0 flex-1"><NavItem icon={Settings} label="设置" to="/settings" /></div><button aria-label="一键启动游戏" className="grid size-9 shrink-0 place-items-center rounded-xl border border-pink-300/20 bg-pink-300/10 text-pink-200 transition hover:bg-pink-300/20 disabled:opacity-50" disabled={starting} onClick={() => void startGame()} title="一键启动游戏" type="button"><Gamepad2 className={`size-[17px] ${starting ? "animate-pulse" : ""}`} /></button></div>
