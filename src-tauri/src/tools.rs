...
pub fn open_local_resource_in_explorer(
    ...
    explorer_select(&candidate)
}

#[cfg(windows)]
fn executable_for(client: LocalClient, state: &AppState) -> CommandResult<PathBuf> {
    let source = state.local_analysis.source_status(client)?;
    let root = source
        .install_root
        .ok_or_else(|| CommandError::new("GAME_NOT_FOUND", "未找到对应 osu! 客户端安装目录"))?;
    let root = PathBuf::from(root);
    let executable = if client == LocalClient::Lazer {
        root.join("current").join("osu!.exe")
    } else {
        root.join("osu!.exe")
    };
...
pub fn set_default_file_client(
    ...
    #[cfg(windows)]
    {
        ...
        let executable = executable_for(client, &state)?;
        let classes = RegKey::predef(HKEY_CURRENT_USER)
